...
let rec eval e (env : (string * int) list) : int =
    ...
    | Var x             -> lookup env x 
    | Let(x, ebody) -> 
      let env1 = List.foldBack(fun e res -> (fst(e),eval (snd(e)) env)::res  ) x env 
      eval ebody env1
...
let rec freevars e : string list =
    ...
    | CstI i -> []
    | Var x  -> [x]
    | Let(x, ebody) -> List.foldBack(fun l res -> union (freevars (snd(l)), minus(freevars ebody, (List.foldBack(fun a r -> fst(a)::r) x [])))) x []        
...
let rec tcomp (e : expr) (cenv : string list) : texpr =
    ...
    | Let((x,e)::xs, ebody) ->  
      let cenv1 = x :: cenv 
      let newLet = Let(xs,ebody) 
      TLet(tcomp e cenv, tcomp newLet cenv1)
    | Prim(ope, e1, e2) -> TPrim(ope, tcomp e1 cenv, tcomp e2 cenv);;
    
   let test = (Let ([("x",CstI 5);("y",CstI 2)], Prim("-", (Prim("+",Var "x", Var "y")), Var "z")));;
